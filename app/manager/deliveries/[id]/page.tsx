...
export const metadata: Metadata = {
  title: "Delivery",
  description: "Exchange record, assignment and reconciliation.",
};

export default async function ManagerDeliveryDetailPage(
  props: PageProps<"/manager/deliveries/[id]">,
) {
  ...
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <Row label="Order">
              <Link
                href={`/manager/orders/${delivery.order.id}`}
            ...
            <Row label="Customer">
              <Link
                href={`/manager/customers/${delivery.order.customer.id}`}
                className="text-brand-600 hover:underline"
              >
      ...
      {delivery.requiresReconciliation && (
        <Card className="mt-6">
          <CardHeader>
            <CardTitle>Reconcile failed delivery</CardTitle>
            <CardDescription>
              Stock is still with the driver. Returning it to the warehouse
              clears the exception.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ReconcileForm deliveryId={delivery.id} />
          </CardContent>
        </Card>
      )}
